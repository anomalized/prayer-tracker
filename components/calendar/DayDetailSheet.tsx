...
const STATUS_STYLE: Record<string, { bg: string; text: string; label: string }> = {
  ontime: { bg: "bg-nude-100",  text: "text-nude-700",  label: "On time" },
    ...
    <>
      ...
      <div
        className="fixed bottom-0 left-0 right-0 z-50 mx-auto max-w-md
          bg-nude-50 rounded-t-3xl animate-slide-up pb-safe"
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            aria-label="Close"
            className="absolute top-3 right-4 w-8 h-8 rounded-xl bg-white/60
              flex items-center justify-center text-nude-500
          ...
          {/* Event badge */}
          {event && (
            <div className="inline-flex items-center gap-1.5 bg-white/70 border
          ...
          {/* Dates */}
          <p className="font-display text-xl font-bold text-nude-800 leading-snug">
            {gregFormatted}
          </p>
          <p className="font-body text-xs text-nude-500 mt-1">{hijriFormatted}</p>

          {/* Arabic event name */}
          {event?.arabicName && (
            <p
              className="text-lg text-nude-600 mt-2 leading-relaxed"
              style={{
                fontFamily:
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-3 px-4 py-3 rounded-2xl
                      ${style ? style.bg : "bg-nude-50"}`}
                  >
                    <span className="text-lg flex-shrink-0" aria-hidden>
                      {meta.icon}
                    </span>
                    <p className={`font-body text-sm font-bold flex-1
                      ${style ? style.text : "text-nude-300"}`}>
                      {meta.label}
                    </p>
