  ...
  return (
    <div className="bg-white border border-nude-100 rounded-3xl overflow-hidden shadow-sm">
      ...
      <div className="divide-y divide-nude-50">
        {sorted.map((entry, i) => {
          const displayName = entry?.name || "Friend";
          const initial = displayName.charAt(0).toUpperCase();
          const rank  = getRank(entry.points ?? 0);
          const color = RANK_COLORS[rank];
          return (
            <div key={entry.id} className={`flex items-center gap-3 px-4 py-3 ${entry.isMe ? "bg-nude-50" : ""}`}>
              ...
              <div className="flex-1">
                <p className="font-body text-sm font-bold text-nude-800">
                  {displayName} {entry.isMe && <span className="text-nude-400 font-normal text-xs">(you)</span>}
                </p>
                <p className="font-body text-xs text-nude-400">streak: {entry.streak ?? 0}</p>
              </div>
